...
def ani_3D_update(i):
    ...
    COM_pos = [COM_pos_x[i], COM_pos_y[i], COM_pos_z[i]]
...
for i in range(int(total_time/delta_t)):
    ...
    if (i > 0) and (i % switch_index == 0):
        ...
        if LIPM_model.support_leg == 'left_leg':
            ...
            swing_foot_pos[1:swing_data_len-1, 2] = (step_num+0.5)*height
            swing_foot_pos[1:15, 2] = np.linspace((step_num-1)*height, (step_num+0.5)*height, 14)
            swing_foot_pos[swing_data_len-10:swing_data_len-1, 2] = np.linspace((step_num+0.5)*height,step_num*height, 9)
        else:
...
for i in range(len(COM_pos_x)):
    COM_pos_z.append(slope*COM_pos_x[i]+COM_pos_0[2])

for i in range(len(COM_pos_x)-1):
    if(left_foot_pos_z[i+1]==0 and left_foot_pos_z[i+1]<left_foot_pos_z[i]):
        left_foot_pos_z[i+1]=left_foot_pos_z[i]
    if(right_foot_pos_z[i+1]==0 and right_foot_pos_z[i+1]<right_foot_pos_z[i]):
        right_foot_pos_z[i+1]=right_foot_pos_z[i]
# ------------------------------------------------- animation plot
